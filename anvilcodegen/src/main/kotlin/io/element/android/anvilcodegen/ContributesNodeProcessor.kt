...
class ContributesNodeProcessor(
    ...
    override fun process(resolver: Resolver): List<KSAnnotated> {
        /**
         * ksp 通过 KSDeclaration表示kotlin中的声明(如类、函数、属性)
         * KSClassDeclaration 是他的子接口，专门表示类或接口的声明 (对应kotlin中class和interface关键字定义的实体)
         */
     ...
     * abstract class ProfileNode_Module {
     *  @Binds
     *  @IntoMap
     *  @NodeKey(ProfileNode::class)
     *  abstract fun bindNodeProfileFactory(factory: ProfileNode_AssistedFactory): AssistedFactory<*>
     * }
     */
    ...
    /**
     * @AssistedFactory
     * interface ProfileNode_AssistedFactory : AssistedNodeFactory<ProfileNode> {
     *  override fun crate(buildContext: BuildContext, plugins: Plugins): ProfileNode
     * }
     */
    ...
    private fun generateFactory(ksClass: KSClassDeclaration) {
        ...
        val constructor = ksClass.getConstructors().singleOrNull { it.isAnnotationPresent(
            AssistedInject::class) }
